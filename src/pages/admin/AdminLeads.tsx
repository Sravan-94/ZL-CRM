...
const AdminLeads = () => {
  ...
  const handleAssignLeads = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('https://crm.infororg.com/api/leads/assign', {
        method: 'POST',
  ...
  const handleBulkDelete = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('https://crm.infororg.com/api/leads/bulk-delete', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          leadIds: Array.from(selectedLeads).map(id => parseInt(id)),
  ...
  const handleImportCSV = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('https://crm.infororg.com/api/leads/upload', {
        method: 'POST',
        body: formData,
      });
